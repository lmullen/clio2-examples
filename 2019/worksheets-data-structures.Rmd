...
```{r}
naval_promotions[["name"]]
naval_promotions$name
```


```{r}
length(unique(naval_promotions$id))
```
...
```{r}
just_captains <- naval_promotions[naval_promotions$rank == "captain", ]
```
...
naval_promotions$year <- year(naval_promotions$date)

naval_promotions[naval_promotions$year == 1800, ]
```
